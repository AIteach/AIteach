...
import com.example.demo.system.mysql.entity.User;
...
import com.example.demo.system.mysql.service.impl.TeacherService;
            ...
            for (int i = 0; i < linkings.size(); i++) {
                if (linkings.get(i).getRearId() == searchNode.get(j).getId()) {
                    ...
                    for (int z = 0; z < nodes.size(); z++) {
                        if (sourceNode.getId() == nodes.get(z).getId()) doubleNode = true;
                        else doubleNode = false;
                    }
                ...
                if (linkings.get(i).getPreId() == searchNode.get(j).getId()) {
                    ...
                    for (int z = 0; z < nodes.size(); z++) {
                        if (targetNode.getId() == nodes.get(z).getId()) doubleNode = true;
                        else doubleNode = false;
                    }
                    if (!doubleNode) nodes.add(targetNode);
                }
            }
        }
        echartjson.setNodes(nodes);
